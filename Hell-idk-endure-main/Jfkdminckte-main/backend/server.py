...
async def dice_play(request: Request, user: dict = Depends(get_current_user), _=rate_limit("games")):
    ...
    # Generate roll from 0 to 99
    roll = random.randint(0, 99)
    
    # Determine win based on direction
    if game_type == "under":
        # Win if roll is less than chance
        is_win = roll < chance
    else:
        # Win if roll is greater than chance
        is_win = roll > chance
    ...
    if is_win and not user.get("is_youtuber"):
        ...
        if potential_win - bet > bank or not should_player_win(rtp, user, multiplier, "dice"):
            is_win = False
...
async def bubbles_play(request: Request, user: dict = Depends(get_current_user), _=rate_limit("games")):
    ...
    # Track RTP statistics for Bubbles
    await track_rtp_stat("bubbles", bet, win if win > 0 else 0)
...
async def tower_step(request: Request, user: dict = Depends(get_current_user), _=rate_limit("games")):
    ...
    # Check if hit bomb (natural probability)
    bombs_in_row = game["bombs"][str(next_row)]
    hit_bomb = column in bombs_in_row
    
    # RTP adjustment - only force loss if:
    # 1. Player didn't hit bomb naturally
    # 2. Potential win exceeds bank OR
    # 3. RTP check fails with difficulty-adjusted chance
    if not hit_bomb and not user.get("is_youtuber"):
        # Difficulty-based RTP adjustment
        # Low difficulty = very rarely force loss (only at high rows for big wins)
        # High difficulty = let natural probability work (already hard)
        difficulty = game["difficulty"]
        
        # Base natural win probability per difficulty
        if difficulty == "low":
            # 3 safe out of 4 = 75% natural win chance
            # RTP should rarely interfere on low difficulty
            rtp_threshold = 0.95  # Only 5% chance to force loss when RTP triggers
        elif difficulty == "medium":
            # 2 safe out of 4 = 50% natural win chance
            rtp_threshold = 0.70  # 30% chance to force loss when RTP triggers
        else:  # high
            # 1 safe out of 4 = 25% natural win chance
            rtp_threshold = 0.50  # 50% chance to force loss when RTP triggers
        
        # Only check RTP for larger potential wins
        should_check_rtp = potential_win > game["bet"] * 3 or potential_win - game["bet"] > bank
        
        if should_check_rtp:
            # RTP check with difficulty adjustment
            rtp_chance = (rtp / 100) * rtp_threshold
            if get_secure_random() > rtp_chance:
                # Force loss - move bomb to player's position
...
async def crash_bet(request: Request, user: dict = Depends(get_current_user), _=rate_limit("games")):
    ...
    # Generate crash point using cryptographically secure RNG with House Edge formula
    r = get_secure_random()  # Use secure random instead of standard random
    if r < 0.99:
        crash_point = 0.99 / (1 - r)
    else:
        crash_point = secure_random.uniform(100, 1000)
...
async def get_crash_result(bet_id: str, request: Request, user: dict = Depends(get_current_user)):
    """Get crash result after game ends"""
    crash_bet = await db.crash_bets.find_one({"id": bet_id, "user_id": user["id"]}, {"_id": 0})
    if not crash_bet:
        raise HTTPException(status_code=404, detail="Ставка не найдена")
    ...
    # Money was already credited in /bet endpoint, just return the result
    user_data = await db.users.find_one({"id": user["id"]}, {"_id": 0, "balance": 1})
...
async def oneplatpay_callback(request: Request):
    ...
    try:
        ...
        if status in [1, 2]:
            ...
            logging.info(f"Payment {merchant_id} completed. User {user['id']} balance updated by {total_amount}")
...
async def cryptobot_callback(request: Request):
    ...
    try:
        ...
        if status == "paid":
            ...
            logging.info(f"CryptoBot: Payment {payment['id']} completed. User {user['id']} balance updated by {total_amount}")
...
async def cryptocloud_callback(request: Request):
    ...
    try:
        ...
        if status == "success":
            ...
            logging.info(f"CryptoCloud: Payment {payment['id']} completed. User {user['id']} balance updated by {total_amount}")
