...
const CrashGame = () => {
  ...
  const startFlightPhase = () => {
    ...
    const animate = () => {
      ...
      // SECURITY: If player has a bet, we DON'T use serverCrashPoint (it's null now)
      // We just show animation and let server decide on cashout
      // For display (when no bet), use displayCrashPoint
      const activeCrashPoint = betIdRef.current 
        ? (newMult > 1000 ? 1000 : null)  // Max 1000x, no early stop
        : displayCrashPointRef.current;
      ...
      if (!betIdRef.current && activeCrashPoint && newMult >= activeCrashPoint) {
      ...
      } else if (betIdRef.current && newMult >= 1000) {
        // Max multiplier reached - auto complete for safety
        setCurrentMult(1000);
        setCrashed(true);
        setGamePhase('crashed');
        ...
        if (!hasProcessedResult && !cashedOutRef.current) {
          hasProcessedResult = true;
          fetchCrashResult(betIdRef.current, 1000);
        }
